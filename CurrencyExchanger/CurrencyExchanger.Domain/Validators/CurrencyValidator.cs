...
public class CurrencyValidator
{
    ...
    {
        if(string.IsNullOrEmpty(code) || !Regex.IsMatch(code, @"^[A-Z]{3}$"))
            throw new ArgumentException("Некорректный код валюты при добавлении в базу данных");
    }

    private void ValidateName(string name)
    {
        if (string.IsNullOrEmpty(name) || !Regex.IsMatch(name, @"^[A-Za-z\s\-]{2,50}$"))
    ...
    {
        if (string.IsNullOrEmpty(sign) || !Regex.IsMatch(sign, @"\S"))
            throw new ArgumentException("Некоррекный знак валюты при добавлении в базу данных");
    }
}
